...
<%!
	static final String DEFAULT_DOCUMENT_TYPE = "Pre-Recruitment Checklist";
	static final String DEFAULT_FILE_NAME = "Pre-RecruitmentChecklist.pdf";
	static final String PROCESS_DEFINITION_NAME = "Recruitment";
...
%>
<%
	log.debug("[PDF]generate_prc.jsp---------------------");
	...
	try {
		...
		validProcessDefinitionName = CDCProperties.getProperty("report.PRC.processName",PROCESS_DEFINITION_NAME);
		reportServerURL = CDCProperties.getProperty("report.server.url", "https://localhost/bizflowadvreport");
		documentType = CDCProperties.getProperty("report.PRC.documentType",DEFAULT_DOCUMENT_TYPE);
		fileName = CDCProperties.getProperty("report.PRC.fileName",DEFAULT_FILE_NAME);
		reportPath = CDCProperties.getProperty("report.PRC.path");
	...
	if(success) {
		try {
			...
			for (int i=0; i<worksheetFiles.length; i++) {
				...
				xrsAttachments.setFieldValueAt(r, "ETCINFO", "AUTO-PRC");
