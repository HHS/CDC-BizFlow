...
public class WorkflowResourceRuleService {

    private KieSession kieSession;

    private static final Logger log = LoggerFactory.getLogger(WorkflowResourceRuleService.class);

    public List<WorkflowResource> getResourceList(WorkflowRuleSearch search, List<WorkflowResource> workflowResources) {

        log.debug("search=" + search);
        kieSession = new DroolsBeanFactory().getKieSession(search.getRuleName() + ".drl");
        kieSession.insert(search);
        kieSession.insert(workflowResources);
